...
> 当前版本：**v1.3.5-2026-07-09**。前端已拆分为模块化 JS，活动视图按运动类型（跑步/徒步/攀岩/骑行）展示专业字段；徒步表格新增「天数」列，支持结束日期与负重（重装/轻装/超轻）展示；攀岩/抱石录入不再强制填写距离；总览保留体能档案基准指标，身体年龄参考指标已移至报告中心；新增 Service Worker 自动检测新版本并提示刷新；视图切换保留 URL hash，刷新后停留在当前标签；移动端小屏下启用底部固定导航栏；**新增增量数据同步：首次全量拉取后，后续刷新只拉取自上次同步以来的变更与删除；离线队列增强：乐观锁冲突检测、自动压缩重复变更、指数退避重试；骨架屏 / Shimmer Loading：数据拉取阶段显示与最终布局近似的占位结构，感知加载更快；v1.0.1 热修：修复 render 时误删骨架屏容器导致刷新无加载态的问题；v1.0.2 热修：修复空缓存时仍走增量同步导致数据被覆盖为空的问题，并在页脚新增「清除缓存」兜底入口；v1.0.3 热修：增量同步后若 gear/routes/segments 仍为空，自动回退到全量同步补全，避免历史损坏缓存导致持续空白；v1.1.0 报告中心增强：新增装备重量分类（Base/Worn/Consumable）环形图、体能等级评估/山力评分、周报/月报周期摘要卡片，并改进 Segment PB 匹配逻辑；v1.2.0 路线详情增强：点击路线「详情」可查看完整统计、分段行程、下撤点/住宿、历史活动记录，若路线配置了可访问的 GPX URL，还会自动解析并绘制 Canvas 海拔剖面图；v1.3.0 报告中心增强：训练指标新增状态评估/CTL 趋势/周 TSS 汇总/活动 TSS 明细，新增训练负荷卡片（ACWR、疲劳评分、8 周距离历史柱状图），Segment PB 增加进步趋势与近期记录，Gear Health 增加磨损比例与维护记录；v1.3.1 交互优化：将身体年龄参考指标从总览统计卡片/体能档案网格移至报告中心，使总览只保留客观基准数据；v1.3.2 体验增强：活动表格点击行改为打开运动类型专属详情弹窗（统计、天气、不适、备注、装备），报告中心身体年龄新增历史趋势折线图；v1.3.3 移动端优化：640px 以下表格自动转为堆叠卡片，所有弹窗全屏化并适配安全区与软键盘，输入框字号提升至 16px 防止 iOS 自动缩放，新增弹窗 Escape 关闭与底层滚动锁定，PWA 支持横屏；v1.3.4 活动详情增强：活动详情弹窗新增 GPX 海拔剖面图，优先使用活动自身 GPX，其次自动匹配同名路线的 GPX，无需离开活动弹窗即可查看轨迹海拔剖面；**v1.3.5 双向跳转：活动详情弹窗新增「查看路线详情」入口，路线详情弹窗的历史活动列表支持点击打开对应活动详情，形成活动 ↔ 路线的快速导航闭环。**
