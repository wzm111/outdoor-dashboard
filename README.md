...
> 当前版本：**v1.8.0-2026-07-16**。v1.8.0 AI 主动周报：打开 AI 助手时自动展示本周训练摘要卡片，支持一键生成详细建议。v1.7.2 AI 助手批量历史导入：支持粘贴多条活动或身体日志，AI 解析成列表后一次性预览并批量创建。v1.7.1 AI 助手自然语言修改/删除：在 v1.7.0 创建/更新基础上，支持用自然语言修改身体日志、活动记录、行程计划的具体字段，以及删除已有记录；所有写入操作仍保留「待办卡片」用户确认。v1.7.0 AI 助手执行：在问答基础上，支持用自然语言创建/更新身体日志、活动记录和行程计划；后端先做意图分类与结构化提取，前端以「待办卡片」形式展示关键字段与冲突提示，用户点击确认后才真正写入数据，避免误操作。v1.6.0 AI 助手：新增「AI 助手」对话视图，支持基于用户户外数据（体能档案、最近活动、身体日志、装备、路线、计划等）进行问答，返回 Markdown 格式回答；后端复用 Moonshot + DeepSeek 双模型并行调用，前端保留最近 50 条对话历史、快捷问题模板、清空对话与重试。
