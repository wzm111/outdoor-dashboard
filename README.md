...
> 当前版本：**v34.6-2026-07-07**。前端已拆分为模块化 JS，活动视图按运动类型（跑步/徒步/攀岩/骑行）展示专业字段；徒步表格新增「天数」列，支持结束日期与负重（重装/轻装/超轻）展示；攀岩/抱石录入不再强制填写距离；新增 Service Worker 自动检测新版本并提示刷新；视图切换保留 URL hash，刷新后停留在当前标签；**移动端小屏下启用底部固定导航栏，常用入口一触即达**。
