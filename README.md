...
> 当前版本：**v34.7-2026-07-07**。前端已拆分为模块化 JS，活动视图按运动类型（跑步/徒步/攀岩/骑行）展示专业字段；徒步表格新增「天数」列，支持结束日期与负重（重装/轻装/超轻）展示；攀岩/抱石录入不再强制填写距离；新增 Service Worker 自动检测新版本并提示刷新；视图切换保留 URL hash，刷新后停留在当前标签；移动端小屏下启用底部固定导航栏；**新增增量数据同步：首次全量拉取后，后续刷新只拉取自上次同步以来的变更与删除，显著减少流量与等待时间**。
