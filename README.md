...
> 当前版本：**v1.7.0-2026-07-15**。v1.7.0 AI 助手执行：在问答基础上，支持用自然语言创建/更新身体日志、活动记录和行程计划；后端先做意图分类与结构化提取，前端以「待办卡片」形式展示关键字段与冲突提示，用户点击确认后才真正写入数据，避免误操作。v1.6.0 AI 助手：新增「AI 助手」对话视图，支持基于用户户外数据（体能档案、最近活动、身体日志、装备、路线、计划等）进行问答，返回 Markdown 格式回答；后端复用 Moonshot + DeepSeek 双模型并行调用，前端保留最近 50 条对话历史、快捷问题模板、清空对话与重试。
