...
function unwrap(row) {
  ...
  if ('goal_type' in row) flat.goal_type = row.goal_type;
  if ('target_value' in row) flat.target_value = row.target_value;
  if ('current_value' in row) flat.current_value = row.current_value;
  if ('unit' in row) flat.unit = row.unit;
